...
| 파이프 길이 $L$ | 5 | |
| 파이프 반지름 $R$ | 0.5 | |
| 막 위치 $x_m$ | 3.75 | 파이프 끝에서 1.25 |
| 유체 밀도 $\rho$ | 1.0 | |
| 유체 점성 $\mu$ | 0.01 | |
| 최대 입구 유속 $u_{max}$ | 10.0 | Poiseuille + sin 프로파일 |
| Brinkman 저항 $R_m = \mu/\kappa$ | 50.0 | 물리 기반은 $\mu/\kappa = 2$ |
| 막 장력 $T$ | 25 | |
...
- 유체 내부에서는 격자가 꼬이지 않도록 부드럽게 재배치됩니다 (Eulerian).
